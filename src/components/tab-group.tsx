  ...
  if (!expanded && tabs.length > SHOW_N_TABS) {
    return (
      <div className="tab-group--expand">
        <a onClick={clickHandler}>+{tabs.slice(SHOW_N_TABS).length} more</a>
      </div>
    );
  }
}
...
const TabGroup: FunctionComponent<Props> = ({
  ...
  return (
    <div {...attrs} className={`tab-group ${className || ""}`}>
      ...
      <ul className="tab-group--urls">{renderTabList(tabs, isExpanded)}</ul>
